...
def waitForPos():
    ...
    while True:
        ...
        fullarray = numpy.array([x,y,z,vx,vy,vz])

        print(fullarray)

        currentstr = f'X: {x}, Y: {y}, Z: {z}, Vx: {vx}, Vy: {vy}, Vz: {vz}'
        currentstr = f'{x} {y} {z} {vx} {vy} {vz}'

#        posstr = data.get("pos")
#        velstr = data.get("vel")
#
#        pos = numpy.frombuffer(posstr)
#        vel = numpy.fromstring(velstr)
#
#        fullarray = numpy.concatenate((pos,vel))
#
#        print(fullarray)
#
#        currentstr = numpy.array2string(fullarray)
